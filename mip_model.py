...
def milp_model(file):
    ...
    parameters = get_parameters("evrptw_instances/c101C10.txt", 2)
    ...
    ####################################################################
    # model.addConstr(sum(X["D0", j] for j in clients + stations) == 3)
    ####################################################################
    ...
    for i in clients + depot_start:
        for j in clients + stations + depot_end:
            if i != j and not (i == "D0" and j == "D0_end"):
                model.addConstr(t[i] + (times[i, j]+service_time[i])*X[i, j] - due_date["D0"]*(1-X[i, j]) <= t[j], name=f"con_time{i}-{j}")

    # arrival time for stations
    for i in stations:
        for j in clients + stations + depot_end:
            if i != j and not (i == "D0" and j == "D0_end"):
                model.addConstr(t[i] + times[i, j]*X[i, j] + g*(Y[i]-y[i]) - (due_date["D0"]+g*Q)*(1-X[i, j]) <= t[j], name=f"con_time{i}-{j}")
    ...
    print(helper.get_routes_dict(binary_arcs), len(helper.get_routes_dict(binary_arcs)))
